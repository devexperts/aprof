...
import com.devexperts.aprof.util.FastFmtUtil;
 ...
 */
public class TextDumpFormatter implements DumpFormatter {
	...
	public TextDumpFormatter(Configuration config) {
		this.config = config;
	...
	private void dumpSnapshotByLocations(PrintWriter out, SnapshotRoot ss, String insideOf) {
		// rebuild locations
		rebuildLocations(ss, insideOf);
		// sort them and print
		locations.sortChildrenDeep(getOutputComparator());
		printLocationsDeep(out, 0, locations, ss);
	}

	protected SnapshotDeep rebuildLocations(SnapshotRoot ss, String insideOf) {
		locations.clearDeep();
